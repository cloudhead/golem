...
this.Server.prototype.init = function () {
    ...
    this.logger.info('listening on ' + (this.settings.host || '0.0.0.0') + ':' + this.settings.port);
    ...
    if (!this.settings.daemonize && !this.settings.raw) {
        tty.setRawMode(true);
    ...
    this.statusInterval = setInterval(function () {
        var ary = [];
        var connections = Object.keys(golem.workers).reduce(function (t, pid) {
            var w = golem.workers[pid];
            return t + (ary[w.number] = w.connections);
        }, 0);
        if (connections !== that.connections) {
            that.logger.info(connections + ' client(s) connected: (' + ary.join(' ') + ')');
...
this.Server.prototype.handleKeyboard = function (code) {
    switch (code) {
        ...
        default:       return code && process.stdout.write(code);
    }
}
    ...
    for (var i = 0, child; i < this.workers; i ++) {
        if (function () { // Skip workers which area already spawned
            for (var k in golem.workers) {
                if (golem.workers[k].number === i &&
                    golem.workers[k].stale  === false) { return true }
            }
        }()) { continue }
...
this.Server.prototype.maintainWorkerCount = function (c) {
    var that = this;
    var pids = Object.keys(golem.workers).filter(function (pid) {
        return !golem.workers[pid].stale;
    });
    var offset = this.workers - pids.length;

    this.logger.info("checking worker count. " + pids.length + " active. offset is " + offset);
...
this.Server.prototype.master = function (worker) {
    ...
    worker.on('data', function (data) {
        ...
        switch (event) {
            ...
            case 'timeout': return worker.kill(posix.SIGTERM);
            case 'ok':      return;
            case 'error':
            default:
        }
...
this.Server.prototype.worker = function (worker, fd) {
    ...
    this.app.on('listening', function () {
        ...
        setInterval(function () {
            that.pipe.writable && that.pipe.write('ok ' + that.connections);
        }, 1500);
    });
...
this.Server.prototype.workerQuit = function () {
    ...
    if (this.app.connections) {
        // This worker is now 'stale', we indicate that
        // through the process name.
        process.title += '*';

        if (this.settings.timeout) {
            setTimeout(function () {
               that.pipe.write('timeout');
            }, this.settings.timeout);
        }
        //
        // Stop accepint connections and close
        // the socket.
        // We could possibly get away with just
        // `this.app.watcher.stop()`; I'm unsure
