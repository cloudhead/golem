...
this.Server.prototype.trap = function (signal, callback) {
    ...
    process.on(signal, function () {
        that.logger.notice((that.isMaster ? 'master' : 'worker[' + process.pid + ']') + ' received ' + signal);
        that.emit(signal);
        callback.call(that);
    });
};
...
this.Server.prototype.master = function (worker) {
    ...
    this.logger.info('initializing worker' + worker.number + '[%d]', worker.pid);

    // Create a new Worker interface, and add it to
    // the `workers` object.
...
// Create a stream for writing to the Master process,
...
this.Server.prototype.worker = function (worker, fd) {
    ...
    this.removeAllListeners('worker');
    this.removeAllListeners('workerExit');
